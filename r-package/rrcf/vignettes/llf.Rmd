...
Last, consider variance estimates and confidence intervals, which are analogous to rrcf variance estimates. We use our first data-generating process for easier visualization.
...
ptm <- proc.time()
preds.rrcf <- predict(forest, X.test)$predictions
mse.rrcf <- mean((preds.rrcf - truth.test)**2)
time.rrcf <- (proc.time() - ptm)[[3]]
...
cat(paste("GRF training took", round(time.train, 2), "seconds. \n",
          ...
          "LLF training took", round(time.rrcf, 2), "seconds. \n",
...
cat(paste("GRF predictions had MSE", round(mse.rrcf, 2), "\n",
