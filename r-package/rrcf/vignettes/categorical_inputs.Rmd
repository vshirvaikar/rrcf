...
In this example, we will illustrate several ways of dealing with categorical variables when using rrcf.

One of the approaches below relies on `rrcf`'s sister package [`sufrep`](https://github.com/rrcf-labs/sufrep). Let's install and load it first.

```{r setup}
library(rrcf)
install.packages("https://github.com/rrcf-labs/sufrep/blob/master/sufrep_0.1.0.tar.gz?raw=true", repos = NULL, type = "source")
library(sufrep)
```
...
+ Use a _sufficient representation_ of the category. Here we will use the `means` method from the [`sufrep`](https://github.com/rrcf-labs/sufrep) package.
