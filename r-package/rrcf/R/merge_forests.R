...
#'                        All forests must be of the same type, and the type must be a subclass of `rrcf`.
...
validate_forest_list <- function(forest_list) {
  ...
  first_forest <- forest_list[[1]]
  if (!methods::is(first_forest, "rrcf")) {
    stop("Argument 'forest_list' must be a list of rrcf objects.
